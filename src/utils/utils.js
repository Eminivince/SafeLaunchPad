...
export const getTokenData = async (tokenAddress, web3) => {
  ...
  return {
    ...
    tokenSymbol,
    totalSupply,
    tokenDenominator: BigNumber(10).pow(BigNumber(parseInt(tokenDecimals))),
  };
};
...
export function getValidImageUrl(imageUrl, infuraDedicatedGateway) {
  if (!infuraDedicatedGateway || !imageUrl) {
    console.error(
      'The app has not "Infura Dedicated Gateway" option or imageUrl is not valid'
    );
    return "";
  }
...
export function getValidIPFSUrl(url, infuraDedicatedGateway) {
  if (!infuraDedicatedGateway || !url) {
    console.error(
      'The app has not "Infura Dedicated Gateway" option or url is not valid'
    );
    return "";
  }
  ...
  return isURL
    ? url.match("gateway.pinata.cloud")
...
export const switchInjectedNetwork = async (chainId) => {
  ...
  try {
    ...
    const messageAboutAddition = JSON.stringify(error).match(
      /(T|t)ry adding the chain/
    );
